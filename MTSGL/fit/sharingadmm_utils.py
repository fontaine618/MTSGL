...
def ridge_saturated(
	...
	while True:
		t += 1
		z_prev = zt
		yt = {task: zt[task] + (zt[task] - ztm1[task]) * (t - 1) / (t + 2) for task in loss.keys()}
		grad = loss.gradient_saturated(yt)
		grad = {task: g + (yt[task] - a[task]) / tau for task, g in grad.items()}
		products = {task: np.matmul(grad[task].transpose(), zt[task] - ztm1[task]) for task in loss.keys()}
		if sum(products.values()) > 0.:
			...
			zt = {task: zt[task] - grad[task] * step_size for task in loss.keys()}
		else:
			# use momentum
			zt = {task: yt[task] - grad[task] * step_size for task in loss.keys()}
		ztm1 = z_prev
		step = np.sqrt(sum([np.linalg.norm(zt[task] - z_prev[task], 2) ** 2 for task in loss.keys()]))
		if t >= max_iter:
			raise RuntimeError("ridge did not converge in {} iterations".format(t))
		if step < threshold:
			break
